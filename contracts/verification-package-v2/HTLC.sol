...
contract HTLC is ReentrancyGuard {
    ...
    struct Contract {
        ...
        bool withdrawn;       // Claimed status
        bool refunded;        // Refunded status
        bytes32 preimage;     // Revealed preimage
    }
    ...
    /**
     * @notice Create a new HTLC for native HBAR (Rebalancing/Marketplace)
     * @dev Anyone with valid preimage can claim (permissionless marketplace)
     * @param _hashlock SHA256 hash of the secret preimage (typically Lightning payment hash)
    ...
    /**
     * @notice Create a new HTLC for ERC20 tokens (Rebalancing/Marketplace)
     * @dev Anyone with valid preimage can claim (permissionless marketplace)
     * @param _hashlock SHA256 hash of the secret preimage (typically Lightning payment hash)
     * @param _timelock Unix timestamp after which sender can refund
    ...
    /**
     * @notice Claim funds by revealing the preimage (Permissionless Marketplace)
     * @dev Anyone can claim EXCEPT the sender (prevents self-front-running)
     * @dev Claimer receives the funds (incentivizes liquidity provision)
     * @dev WARNING: Invoice creator could front-run with different address
        ...
        withdrawable(_contractId)
    {
        ...
        // SECURITY: Sender cannot claim their own HTLC
        // Prevents basic attack where sender immediately claims back
        // NOTE: Sender could still use alt address (accepted risk for permissionless design)
        require(
            msg.sender != c.sender,
            "HTLC: Sender cannot claim own HTLC"
        );
        
        c.withdrawn = true;
        c.preimage = _preimage;

        // Transfer to claimer (who paid the Lightning invoice)
        if (c.tokenAddress == address(0)) {
            // Native HBAR transfer to claimer
            (bool success, ) = msg.sender.call{value: c.amount}("");
            require(success, "HTLC: Native transfer failed");
        } else {
            // ERC20 token transfer to claimer
            IERC20(c.tokenAddress).safeTransfer(msg.sender, c.amount);
        }

        emit HTLCClaimed(_contractId, msg.sender, _preimage);
    }

    /**
     * @notice Refund locked funds after timelock expires
     * @dev Can be called by anyone (permissionless auto-refund)
     * @dev Funds always return to original sender
     * @param _contractId The HTLC contract identifier
     */
        ...
        refundable(_contractId)
    {
        ...
        // Always refund to original sender
        if (c.tokenAddress == address(0)) {
